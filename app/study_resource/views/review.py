...
class InternalReviewVieset(VotableVieset):
    serializer_class = serializers.InternalReviewSerializer
    queryset = serializers.InternalReviewSerializer.queryset.order_by('-created_at')
    permission_classes = [IsAuthenticatedOrReadOnly]
    ...
    def perform_destroy(self, instance):
        super(InternalReviewVieset, self).perform_destroy(instance)
        self.request.user.positive_score -= rewards.REVIEW_DELETE
        create_notification(instance.study_resource._meta.model, instance.study_resource.pk,
                            self.request.user.pk,
                            notification_events.VERB_REVIEW_REMOVE)
