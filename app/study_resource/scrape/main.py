...
def scrape_tutorial(url):
    ...
    result["publishing_date"] = (
        article.publish_date.strftime(constants.DATE_FORMAT)
        if type(article.publish_date) == datetime.datetime
        else datetime.date.today().strftime(constants.DATE_FORMAT)
    )
    result["created_by"] = article.authors
    result["summary"] = "\n\n----\n\n".join(
        [s for s in [article.summary, article.meta_description] if s]
    )
    return result
