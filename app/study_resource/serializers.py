...
from .models import (StudyResource, Review, InternalReview, StudyResourceImage, 
StudyResourceTechnology, StudyResourceIntermediary, InternalStudyResourceTechnology, 
InternalStudyResource, InternalStudyResourceImage)
...
class InternalReviewSerializer(serializers.ModelSerializer):
    queryset = InternalReview.objects
    study_resource = StudyResourceListingMinimalSerializer
    author = UserSerializerMinimal(many=False, read_only=True)
...
class InternalImageSerializer(serializers.ModelSerializer):
    queryset = InternalStudyResourceImage.objects
    image_file = VersatileImageFieldSerializer(
        sizes=settings.VERSATILEIMAGEFIELD_RENDITION_KEY_SETS['resource_image'],
        read_only=True
    )
...
class InternalStudyResourceEditSuggestionListingSerializer(serializers.ModelSerializer):
    queryset = InternalStudyResource.edit_suggestions.all()
    edit_suggestion_author = UserSerializerMinimal(read_only=True)

    class Meta:
        model = InternalStudyResource.edit_suggestions.model
        fields = ['pk',
                  'edit_suggestion_reason', 'edit_suggestion_author', 'edit_suggestion_date_created',
                  'thumbs_up', 'thumbs_down']
...
class InternalStudyResourceEditSuggestionSerializer(serializers.ModelSerializer):
    queryset = InternalStudyResource.edit_suggestions.all()
    edit_suggestion_author = UserSerializerMinimal(read_only=True)
    changes = serializers.SerializerMethodField()
    ...
    def get_changes(self, instance):
        ...
        for change in delta.changes:
            if change.field == 'tags':
                ...
                result.append({'field': change.field.capitalize(),
                               'old': ', '.join([f'{t.name} {t.version}' for t in
                                                 delta.old_record.internalstudyresourcetechnology_set.all()]),
                               'new': ', '.join([
                                   f'{t.name} {t.version}' for t in
                                   instance.technologies.through.objects.filter(study_resource=instance.pk).all()
                               ])
...
class InternalStudyResourceImageSerializer(serializers.ModelSerializer):
    image_file = VersatileImageFieldSerializer(
        sizes=settings.VERSATILEIMAGEFIELD_RENDITION_KEY_SETS['resource_image'],
        required=False,
    )
...
class InternalStudyResourceSerializer(EditSuggestionSerializer):
    ...
    author = UserSerializerMinimal(many=False, read_only=True)
    tags = TagSerializerOption(many=True, read_only=True)
    technologies = StudyResourceTechnologySerializer(source='internalstudyresourcetechnology_set', many=True, read_only=True)
    category_concepts = CategoryConceptSerializerOption(many=True, read_only=True)
    technology_concepts = TechnologyConceptSerializerOption(many=True, read_only=True)
    rating = FloatField(read_only=True)
    reviews_count = IntegerField(read_only=True)
    images = InternalStudyResourceImageSerializer(many=True)
    category = CategorySerializerOption(read_only=True)

    class Meta:
        model = InternalStudyResource
        fields = ['pk', 'rating', 'reviews_count', 'absolute_url', 'name', 'slug', 'content', 'price', 'media',
                  'experience_level', 'author', 'tags', 'category', 'category_concepts', 'technology_concepts',
                  'technologies', 'created_at', 'updated_at', 'images']

    @staticmethod
    def get_edit_suggestion_serializer():
        return InternalStudyResourceEditSuggestionSerializer

    @staticmethod
    def get_edit_suggestion_listing_serializer():
        return InternalStudyResourceEditSuggestionListingSerializer

    def run_validation(self, data):
        data = data.copy()
        if 'slug' not in data:
            data['slug'] = slugify(data['name'])
        validated_data = super(InternalStudyResourceSerializer, self).run_validation(data)
        # get technologies out of the data
        cleaned_technologies = json.loads(data['technologies'])
        validated_data['category_id'] = int(data['category']) if 'category' in data else None
        validated_data['tags'] = Tag.objects.validate_tags(json.loads(data['tags']))
    ...
    def create(self, validated_data):
        ...
        created_instance = super(InternalStudyResourceSerializer, self).create(validated_data)
        self.handle_m2m_fields(created_instance, m2m_fields)
        return created_instance
    ...
    def update(self, instance, validated_data):
        ...
        updated_instance = super(InternalStudyResourceSerializer, self).update(instance, validated_data)
        updated_instance.internalstudyresourcetechnology_set.all().delete()
        self.handle_m2m_fields(updated_instance, m2m_fields)
        return updated_instance
    ...
    def handle_m2m_fields(self, instance, m2m_fields):
        ...
        for tech in techs:
            tech_post_data = list(filter(lambda t: t['technology_id'] == tech.pk, m2m_fields['technologies']))[0]
            InternalStudyResourceTechnology.objects.create(
                study_resource=instance,
                technology=tech,
                version=tech_post_data['version'],
            )
        if 'images' in m2m_fields:
            for img_data in m2m_fields['images']:
                image = InternalStudyResourceImage(internal_study_resource=instance, image_url=img_data['image_url'], line=img_data["line"])
                image.save()

class InternalStudyResourceListingMinimalSerializer(serializers.ModelSerializer):
    queryset = InternalStudyResource.objects.all()
    rating = FloatField(read_only=True)
    reviews_count = IntegerField(read_only=True)
...
class InternalResourceReviewSerializer(serializers.ModelSerializer):
    queryset = InternalReview.objects
    author = UserSerializerMinimal(many=False, read_only=True)
