...
export default function Thumbs({thumbs_obj, url_endpoint, user_id}) {
        ...
        function handleVoteError(result) {
            if (result.status === 403) {
                setAlert(<Alert text="Only signed in users can rate reviews" type="danger" hideable={false} stick={false}/>)
            } else {
                result.json().then(data =>
                    setAlert(<Alert text={data.error ? data.error : data.detail } type="danger" hideable={false} stick={false}/>)
                )
            }
        }
